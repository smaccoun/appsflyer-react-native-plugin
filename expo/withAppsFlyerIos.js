...
function modifyAppDelegate(appDelegate) {
  ...
  if (
    appDelegate.includes(RNAPPSFLYER_CONTINUE_USER_ACTIVITY_IDENTIFIER) &&
    !appDelegate.includes(RNAPPSFLYER_CONTINUE_USER_ACTIVITY_CODE)
  ) {
    const block =
      RNAPPSFLYER_CONTINUE_USER_ACTIVITY_IDENTIFIER +
      "\n" +
    ...
    WarningAggregator.addWarningIOS(
      ...
      "Failed to detect continueUserActivity in AppDelegate or AppsFlyer's delegate method already exists"
    );
  }
  if (
    appDelegate.includes(RNAPPSFLYER_OPENURL_IDENTIFIER) &&
    !appDelegate.includes(RNAPPSFLYER_OPENURL_CODE)
  ) {
    const block =
      RNAPPSFLYER_OPENURL_IDENTIFIER + "\n" + RNAPPSFLYER_OPENURL_CODE;
    appDelegate = appDelegate.replace(RNAPPSFLYER_OPENURL_IDENTIFIER, block);
  } else {
    WarningAggregator.addWarningIOS(
      "withAppsFlyerAppDelegate",
      "Failed to detect openURL in AppDelegate or AppsFlyer's delegate method already exists"
    );
  }
  return appDelegate;
}
...
function withAppsFlyerAppDelegate(config) {
  return withAppDelegate(config, (config) => {
    if (["objc", "objcpp"].includes(config.modResults.language)) {
      ...
      WarningAggregator.addWarningIOS(
        "withAppsFlyerAppDelegate",
        `${config.modResults.language} AppDelegate file is not supported yet`
      );
    }
    return config;
  });
...
function withPlistAdvertisingAttribution(config) {
  return withInfoPlist(config, async (config) => {
    ...
    infoPlist.NSAdvertisingAttributionReportEndpoint =
      RNAPPSFLYER_ATTRIBUTION_ENDPOINT;

    return config;
  });
}

module.exports = function withAppsFlyerIos(
  config,
  shouldUseStrictMode,
  skanEndpoint
) {
